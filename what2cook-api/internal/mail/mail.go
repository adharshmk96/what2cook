...
func (m *Mailer) SendPasswordReset(toEmail, rawToken string) error {
	...
	return m.sendOrLog(
		...
		link,
		"Reset link: %s",
		"Reset your what2cook password",
		fmt.Sprintf("Reset your password using this link (expires soon):\r\n\r\n%s\r\n", link),
		"password reset email sent to %s",
	)
}

// SendEmailVerification delivers a verify link via SMTP, or logs it when SMTP host is empty.
func (m *Mailer) SendEmailVerification(toEmail, rawToken string) error {
	link := fmt.Sprintf("%s/app/verify-email?token=%s", m.publicURL, rawToken)
	return m.sendOrLog(
		toEmail,
		link,
		"Verify link: %s",
		"Verify your what2cook email",
		fmt.Sprintf("Verify your email using this link (expires soon):\r\n\r\n%s\r\n", link),
		"email verification sent to %s",
	)
}

func (m *Mailer) sendOrLog(toEmail, link, logFmt, subject, body, sentLogFmt string) error {
	if strings.TrimSpace(m.smtp.Host) == "" {
		log.Printf(logFmt, link)
		return nil
	}
