...
DEPLOY_PATH="${DEPLOY_PATH:-/opt/what2cook}"
DEPLOY_DOMAIN="${DEPLOY_DOMAIN:-what2cook.${DEPLOY_HOST}.nip.io}"
DEPLOY_SSH_KEY="${DEPLOY_SSH_KEY:-}"
STACK_NAME="${STACK_NAME:-what2cook}"
...
APP_VERSION="${WHAT2COOK_APP_VERSION:-$(git -C "${ROOT_DIR}" describe --tags --always 2>/dev/null || echo dev)}"
DEPLOY_MODE="pull"
...
if [[ -z "${IMAGE_API}" ]]; then
  DEPLOY_MODE="build"
  IMAGE_API="what2cook-api:${RELEASE_ID}"
fi
...
tar \
  ...
  --exclude=what2cook-ui/dist \
  --exclude=what2cook-api/what2cook \
  -C "${ROOT_DIR}" -cf - . \
...
mkdir -p "${deploy_path}"
secret_file="${deploy_path}/auth_token_secret"
if [[ ! -f "${secret_file}" ]]; then
  openssl rand -hex 32 >"${secret_file}"
  chmod 600 "${secret_file}"
fi
auth_token_secret="$(tr -d '[:space:]' <"${secret_file}")"
[[ -n "${auth_token_secret}" ]] || { echo "auth token secret is empty" >&2; exit 1; }
...
export DOMAIN="${domain}" IMAGE_API="${image_api}" WHAT2COOK_AUTH_TOKEN_SECRET="${auth_token_secret}"
...
log "Waiting for https://${DEPLOY_DOMAIN}/healthz"
for attempt in {1..30}; do
  if curl --fail --silent --show-error --max-time 10 "https://${DEPLOY_DOMAIN}/healthz" >/dev/null 2>&1; then
    log "Deployment complete: https://${DEPLOY_DOMAIN}"
    exit 0
  fi
  if (( attempt == 30 )); then
    # Stack is already live; do not fail (or roll back) on a late/flaky probe.
    "${SSH[@]}" "docker service ps '${STACK_NAME}_api' --no-trunc; docker service logs --tail 50 '${STACK_NAME}_api'" || true
    printf '\033[1;33mwarn:\033[0m https://%s/healthz did not respond in time; leaving stack %s deployed\n' \
      "${DEPLOY_DOMAIN}" "${STACK_NAME}" >&2
    exit 0
  fi
  sleep 2
done
